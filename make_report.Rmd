...
```{r load_scripts, include = FALSE, results = 'hide'}
source(file = here("set_project.R"), verbose = FALSE)
source(file = here("set_functions.R"), verbose = FALSE)
source(file = here("make_model.R"), verbose = FALSE)
```
...
**Please note that this is a shorter version adapted for the purpose of illustrating my work as part of the interview for the data scientist position at Nottinghamshire Healthcare NHS Foundation Trust. This version utilised synthetic data (using the synthpop package) and the report does not show demographic variables.**
...
knitr::include_graphics(here("all_plots/report/dag_updated_m1_UEQ.png"), auto_pdf = TRUE)
```
...
```{r correlation, out.width="100%", fig.cap="Correlation"}
knitr::include_graphics(here("all_plots/report/correlation_categorical.png"), auto_pdf = TRUE)
```
...
```{r coercion, out.width="100%", fig.cap="Collapssing the levels"}
knitr::include_graphics(here("all_plots/report/pre_post_levels_merged.png"), auto_pdf = TRUE)
```
...
```{r predictors, out.width="100%", fig.cap="Collapssing the levels"}
knitr::include_graphics(here("all_plots/report/predictors_UEQ_Attractiveness_vars.png"), auto_pdf = TRUE)
```
...
```{r aic_bic, out.width="100%", fig.cap="Collapssing the levels"}
knitr::include_graphics(here("all_plots/report/aicbic_UEQ_Attractiveness_mod.png"), auto_pdf = TRUE)
```
...
```{r diagnostics_1, out.width="100%", fig.cap="Collapssing the levels"}
knitr::include_graphics(here("all_plots/report/diagnostic_1_UEQ_Attractiveness.png"), auto_pdf = TRUE)
```
...
```{r diagnostics_2, out.width="100%", fig.cap="Collapssing the levels"}
knitr::include_graphics(here("all_plots/report/diagnostic_2_UEQ_Attractiveness.png"), auto_pdf = TRUE)
```
...
Figure (below) shows the differences in attractiveness scores between the letter versions. The visualisation indicates that participants favoured the HW version and somewhat less conclusively the control version of the letter as opposed to the other variables.

```{r final_model_visual, out.width="100%", fig.cap="Collapssing the levels"}
knitr::include_graphics(here("all_plots/report/final_vis_UEQ_Attractiveness.png"), auto_pdf = TRUE)
```
